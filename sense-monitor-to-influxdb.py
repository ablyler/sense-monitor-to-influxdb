...
def realtime_callback(payload):
    ...
    for device in payload['devices']:
        records.append(Point("sense_devices").tag("monitor_name", sense_monitor_name).tag("id", device['id']).tag('name', device['name']).field("watts", device['w']))
    for index, channel in enumerate(payload['channels']):
        records.append(Point("sense_channel").tag("monitor_name", sense_monitor_name).tag("channel", index).field("watts", channel))
